...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.maxPool3d_5a_2x2 = MaxPool3dTFPadding(
            kernel_size=(2, 2, 2), stride=(2, 2, 2), padding='SAME') # stride(1,2,2) to improve the temporal resolution
        ...
        if self.with_classifier:
            ...
            if self.projection:
                print('NO classifier, WITH projection MLP!!!')
                ...
                print('NO classifier, NO projection MLP!!!')
    ...
    def forward(self, inp, option=None):
        # print(inp.size())
        # features = []
        out = self.conv3d_1a_7x7(inp) # temporal 2x downsampling
        ...
        # features.append(out)
        out = self.maxPool3d_4a_3x3(out) # temporal 2x downsampling
        out = self.mixed_4b(out)
        out = self.mixed_4c(out)
        out = self.mixed_4d(out)    # [bs,512,4,14,14]
        out = self.mixed_4e(out)    # [bs,528,4,14,14]
        out = self.mixed_4f(out)    # [bs,832,4,14,14]
        # features.append(out)
        feature_map = self.maxPool3d_5a_2x2(out) # [bs,832,2,7,7] 
        out = self.mixed_5b(feature_map)
        out = self.mixed_5c(out)
        
        # out: batch_sizex1024x(number_clip_frame/8)x7x7(crop_size=224)

        if option == 'early_return':
            return out
        if self.with_classifier:
            out = self.avg_pool(out)
            out = out.flatten(1)
            out = self.dropout(out)
            # out = F.normalize(out) ### turn on when linear probe mode
            out = self.fc(out)
            return F.log_softmax(out, dim=1)
        else:
            # out: batch_sizex1024x(number_clip_frame/8)x7x7(crop_size=224)
            # out_proj： batch_sizex1024x1x1x1
            out_proj = self.id_head(out)
            return out, out_proj
