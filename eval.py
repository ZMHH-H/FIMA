...
from dataset import transform_eval
import moco.loader
...
parser.add_argument('-a', '--arch', metavar='ARCH', default='I3D',
                    choices=model_names,
...
parser.add_argument('-cpv', '--clip_per_video', default=10, type=int, metavar='N',
                    help='number of clip per video video (default: 10)')
...
parser.add_argument('--ori_resolution', default=256, type=int, metavar='N',
                    help='the short edge of original dataset')
best_acc1 = 0
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.num_class == 101:
        ...
        if args.finetune:
            args.ckp_dir = os.path.join(args.log_dir, "finetune", "checkpoints_UCF")
            args.log_dir = os.path.join(args.log_dir, "finetune", "logs_UCF")
        args.dropout = 0.7
    elif args.num_class == 51:
        if args.freeze:
            args.ckp_dir = os.path.join(args.log_dir, "linear", "checkpoints_HMDB")
            args.log_dir = os.path.join(args.log_dir, "linear", "logs_HMDB")
        if args.finetune:
            args.ckp_dir = os.path.join(args.log_dir, "finetune", "checkpoints_HMDB")
            args.log_dir = os.path.join(args.log_dir, "finetune", "logs_HMDB")
        args.dropout = 0.5
    ...
    if args.arch == "r2plus1d_18":
        # Todo: check
        model = R2PLUS1D(num_classes=args.num_class, drop=args.finetune, dropout=args.dropout,
                        with_classifier=True, projection=False)
        if args.freeze:
            model = R2PLUS1D(num_classes=args.num_class, drop=False, with_classifier=True, projection=False)
        ...
        model = I3D(num_classes=args.num_class, dropout_prob=args.dropout, with_classifier=True, projection=False)
        if args.freeze:
            model = I3D(num_classes=args.num_class, dropout_prob=0, with_classifier=True, projection=False)
            for name, param in model.named_parameters():
                if name not in ['fc.weight', 'fc.bias']:
                    param.requires_grad = False
    ...
    # optimize only the linear classifier or all parameters
    ...
    video_augmentation_val = transforms.Compose(
        [
            transforms_video.ToTensorVideo(),
            transforms_video.CenterCropVideo(args.ori_resolution*0.875), # 256*0.875=224
            transform_eval.VideoClipScale(args.crop_size),
            normalize_video,
        ]
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch % 10 == 0 or epoch == args.epochs - 1) and (not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, ckp_dir, max_save=3, is_best=is_best)
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (video, target) in enumerate(val_loader):
            if args.gpu is not None:
                video = video.cuda(args.gpu, non_blocking=True) # [cpv,3,fpv,H,W]
                target = target.cuda(args.gpu, non_blocking=True)
            
            # compute output
            output = model(video)   # clip_per_video x num_class
            output = output.view(-1, args.clip_per_video, args.num_class) # 1 x clip_per_video x num_class
            target = target.view(-1, args.clip_per_video)   # 1 x clip_per_video
            output = torch.mean(output, dim=1)

            # make sure 10 clips belong to the same video
            for j in range(1, args.clip_per_video):
                assert all(target[:, 0]==target[:, j])
            target = target[:, 0]   # torch.tensor([label])
            loss = criterion(output, target)
